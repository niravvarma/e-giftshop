...
<script type="text/javascript">
	var translateAssetTab = function(DimId, DimType){
		window.parent.fw.ui.app.translate({
			dimension:{
				id: DimId,
				type: DimType
			}
	...
	var deleteAssetT = function(aId, aType, aName){
		...
		window.parent.fw.ui.app.onContextMenu(data) ; 
	};
	...
	<tr>
		...
		<td >
			...
			<tr>
				...
				<td class="tile-b" background="<%=ics.GetVar("cs_imagedir")%>/graphics/common/screen/grad.gif">&nbsp;&nbsp;&nbsp;&nbsp;</td>
				<td class="tile-b" background="<%=ics.GetVar("cs_imagedir")%>/graphics/common/screen/grad.gif">
					<DIV class="new-table-title">Action</DIV>
				</td>
...
for ( IList row : new IterableIListWrapper( ics.GetList("listLocales") ) )
{
    ...
    else if (!bRevisionInspect)
    {
		...
		<% if("ucform".equals(ics.GetVar("cs_environment"))){%>			
			<ics:setvar name="referURL" value='<%="javascript:translateAssetTab(\'" + row.getValue("id") + "\',\'Dimension\');"%>' />
		<% } else { %>
		...
		<% } %>     
			...
			<td>
				<br/>
			</td><td><br/></td><td><span class="disabledText">Not Available</span></td><td><br/></td><td><em><string:stream variable="localeDesc"/></em></td><td><br/></td>
