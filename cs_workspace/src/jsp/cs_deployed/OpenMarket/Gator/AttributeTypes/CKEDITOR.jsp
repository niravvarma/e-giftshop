...
%>
	<ics:getproperty name="xcelerate.ckeditor.basepath"
		file="futuretense_xcel.ini" output="basepath" />
	<ics:getproperty name="ft.cgipath" file="futuretense.ini"
		output="cgipath" />
	<%
...
%>
	<ics:if condition='<%=!Utilities.goodString(ics.GetVar("basepath"))%>'>
		<ics:then>
			<ics:setvar name="basepath"
				value='<%=ics.GetVar("cgipath") + "ckeditor/"%>' />
		</ics:then>
	</ics:if>
	...
	if (!isSingleValued && !"true".equals(ics.GetVar("renderMultiValWidget"))) { %>
			<ics:if condition='<%="true".equals(ics.GetVar("RequiredAttr"))%>'>
				<ics:then>
					<ics:resolvevariables
						name="Variables.RequireInfo*Variables.cs_MultipleInputName*Variables.currentAttrName*ReqTrue*Variables.AttrType!"
						output="RequireInfo" />
				</ics:then>
				<ics:else>
					<ics:resolvevariables
						name="Variables.RequireInfo*Variables.cs_MultipleInputName*Variables.currentAttrName*ReqFalse*Variables.AttrType!"
						output="RequireInfo" />
				</ics:else>
			</ics:if>
			<ics:if condition="<%=isUrlField%>">
				<ics:then>
					...
					<input type="hidden"
						...
						id="<ics:getvar name="cs_MultipleInputName"/>_file"
						value="<%=ics.genID(true)%>.txt" />
					<ics:removevar name="currentValue" />
					<%
                    } else {%>
					<ics:listget listname="AttrValueList" fieldname="urlvalue"
						output="filename" />
					<ics:listget listname="AttrValueList" fieldname="@urlvalue"
						output="currentValue" />
					<ics:if
						condition='<%=!Utilities.goodString(ics.GetVar("filename"))%>'>
						<ics:then>
							<ics:setvar name="filename" value='<%=ics.genID(true) + ".txt"%>' />
						</ics:then>
						<input type="hidden"
							name="<ics:getvar name="cs_MultipleInputName"/>_file"
							id="<ics:getvar name="cs_MultipleInputName"/>_file"
				...
				<ics:else>
					<%
					ics.SetVar("currentValue", hasValues ? attributeValueList.getValue("value") : "");
					%>
				</ics:else>
			</ics:if>
			<%String currentValue = ics.GetVar("currentValue");
                currentValue = (currentValue == null?"":currentValue);
				%>
			<ics:setvar name="currentValue" value='<%=currentValue%>' />
			<ics:setvar name="currentInput"
				value='<%=ics.GetVar("cs_MultipleInputName")%>' />
			<% 
				
				String tempVal = com.openmarket.basic.seed.TextFormat.EncodeString((String)ics.GetVar("currentValue"));
	...
	%>
			<tr>
				...
				<td></td>
				<td><ics:if condition="<%=isSingleValued%>">
					<ics:then>
						<ics:if condition="<%=isUrlField%>">
							<ics:then>
                ...
                attributeValue = (attributeValue == null?"":attributeValue);%>
						...
						<ics:setvar name="currentInput"
							value='<%=ics.GetVar("cs_SingleInputName")%>' />
					</ics:then>
					<ics:else>
					</ics:else>
				</ics:if> <ics:if condition="<%=allowEmbeddedLinks%>">
					<ics:then>
						<ics:if
							condition='<%=!"true".equals(ics.GetVar("embeddedlink_inited"))%>'>
							<ics:then>
				...
				</ics:if> <satellite:link
					pagename="OpenMarket/Xcelerate/Actions/FCKEditorRenderer"
					assembler="query" outstring="urlFCKEditorRenderer">
					<satellite:argument name="cs_environment"
						value='<%=ics.GetVar("cs_environment")%>' />
					<satellite:argument name="cs_formmode"
						value='<%=ics.GetVar("cs_formmode")%>' />
					<satellite:argument name="cs_imagedir"
						value='<%=ics.GetVar("cs_imagedir")%>' />
				</satellite:link> <% 
		...
		%> <!--
		 ...
		 --> <% if (isSingleValued) {
            ...
            %>
				...
				var langAndCountry = '<ics:getssvar name="locale"/>'.toLowerCase().split('_');
				CKEDITOR.config.DefaultLanguage = langAndCountry[0];
                ...
                <ics:if condition='<%="false".equals(ics.GetVar("LAZYLOAD"))%>'><ics:then>
                    <%-- replace textarea immediately with
                         the new ckeditor --%>
             ...
             <% 
			} else if ("true".equals(ics.GetVar("renderMultiValWidget"))) {
		%> <%
				String showSiteTree = "";				
				if (null != ics.GetVar("showSiteTree")) {
				...
				String editorParams = "{'config': {'csRootContext': '" + ics.GetVar("cgipath") + "'," +
												  ...
												  "'enableEmbeddedLinks': '" + ics.GetVar("enableEmbeddedLinks") + "'," +
			...
			%> <ics:callelement
					...
					<ics:argument name="editorName"
						value="fw.ui.dijit.form.FormCKEditor" />
					<ics:argument name="editorParams" value='<%= editorParams %>' />
					<ics:argument name="multiple" value="true" />
					<ics:argument name="isUrlField"
						value="<%= String.valueOf(isUrlField) %>" />
					<ics:argument name="maximumValues" value="<%= maximumValues %>" />
				</ics:callelement> <% 	
				ics.RemoveVar("MAXVALUES");
			} 
		 %> <ics:if
					condition='<%="multiple-ordered".equals(ics.GetVar("EditingStyle"))%>'>
					<ics:then>
						<ics:if condition='<%=ics.GetVar("__fckeditorscripts") == null%>'>
							<ics:then>
								<ics:callelement
									element="OpenMarket/Xcelerate/Scripts/MoveMulFields" />
								<ics:setvar name="__fckeditorscripts" value="true" />
							</ics:then>
						</ics:if>
					</ics:then>
				</ics:if> <%
            	//ics.SetVar("tempval", (String)ics.GetVar("currentValue"));
            %> <ics:removevar name="currentValue" /></td>
				<td></td>
			</tr>
			<%}%>
		</ics:then>
		<ics:else>
			<script type="text/javascript"
				src="<ics:getvar name="basepath"/>ckeditor.js"></script>
		</ics:else>
	</ics:if>
</cs:ftcs>
