    ...
    <ics:if condition='<%=ics.GetVar("locale") != null%>'>
        <ics:then>
            ...
            <ics:if condition='<%=ics.GetVar("asset:locale:OBJECTID") == null || ics.GetVar("locale").equals(ics.GetVar("asset:locale:OBJECTID"))%>'>
                <ics:then>
                    <ics:logmsg 
                                msg='<%="About to filter and look up translation for asset "+ics.GetVar("c")+":"+ics.GetVar("cid")+" because it either does not have a locale associated with it or it is already in the requested locale"%>'/>
                ...
                <ics:else>
                    ...
                    <ics:logmsg msg='<%="Filter complete.  Filter returned asset "+ics.GetVar("c")+":"+ics.GetVar("cid")+"."%>'/>

                </ics:else>
            </ics:if>
        </ics:then>
        <ics:else>
            <ics:logmsg msg='<%="Do not need to filter or look up translation for asset "+ics.GetVar("c")+":"+ics.GetVar("cid")+" because no preferred locale was specified."%>'/>
