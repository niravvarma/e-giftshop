...
%><cs:ftcs><%-- Locale/ChangeLocalePost
...
<%-- The user wants to change the locale... so change it in the session
    ...
    <%-- Now that the locale drop-down uses gettemplateurl
    ...
    <ics:if condition='<%=ics.GetVar("preferred_locale").equals(ics.GetSSVar("preferred_locale"))%>'>
        ...
        <ics:else>
            ...
            <%-- Then, look up the translation of the Home page that corresponds to
                 ...
                 --%>
            <render:lookup site='<%=ics.GetVar("site")%>' ttype="CSElement" tid='<%=ics.GetVar("eid")%>' key="HomePage" varname="HomePageName" match=":x"/>
            <asset:load name="HomePage" type="Page" field="name" value='<%=ics.GetVar("HomePageName")%>' flushonvoid="true" />
